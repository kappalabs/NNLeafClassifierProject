...
train_data, train_labels, test_data, test_labels, train_descriptors, test_descriptors = leaf_reader.readTrainingData(
    update_descriptors=True)
    # update_descriptors=False)

# print(np.shape(train_data))
# print(np.shape(train_descriptors[::, 0, ::]))
# train_data = np.hstack((train_data, train_descriptors[::, 0, ::]))
# test_data = np.hstack((test_data, test_descriptors[::, 0, ::]))
...
nn = models.ConvNet1D2(filters=200, filters2=120, n_hidden=150)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        # Count & save accuracy of current epoch
        ...
        if (epoch + 1) % display_step == 0:
            print("Epoch:", '%04d' % epoch, "training cost=", "{:.9f}".format(avg_cost),
                  "accuracy=", "{:.9f}".format(acc), "best accuracy: ", "{:.3f}".format(best_accuracy))
